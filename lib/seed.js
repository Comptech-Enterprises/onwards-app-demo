...
export const LOCATIONS = [
  ...
  "E-40/7, Okhla Phase II",
  "Noida Sector 126",
  "Udyog Vihar Phase 4",
  "Emaar Capital",
  "Mohan Cooperative",
  "ECE House, Connaught Place",
  "Nuvama, Connaught Place",
];
...
export const MAX_PANTRY_PHOTOS = 4;
export const MIN_WASHROOM_PHOTOS = 3;
export const MAX_WASHROOM_PHOTOS = 8;

export const CHECKLIST_PHOTO_GATES = {
  Pantry: {
    min: 1,
    max: MAX_PANTRY_PHOTOS,
    label: "pantry",
    hint: "Wide-angle photos of the entire pantry, one per pantry at this centre. Up to 4. Required before any pantry task can be ticked.",
  },
  Washroom: {
    min: MIN_WASHROOM_PHOTOS,
    max: MAX_WASHROOM_PHOTOS,
    label: "washroom",
    hint: "Wide-angle image of each washroom, minimum 3 per building. Required before any washroom task can be ticked.",
  },
  "Common Areas": {
    min: 2,
    max: 4,
    label: "common area",
    hint: "Photos of the entrance and the reception desk. Both required before any common-area task can be ticked.",
  },
  "Soft Services": {
    min: 1,
    max: 1,
    label: "soft services staff",
    hint: "One photo of the soft services staff — 2–3 people together in frame. Required before any soft-services task can be ticked.",
  },
};

export function photoList(raw) {
  if (Array.isArray(raw)) return raw.filter(Boolean);
  return (raw?.slots || []).filter(Boolean);
}

export function checklistPhotosReady(category, photos) {
  const gate = CHECKLIST_PHOTO_GATES[category];
  if (!gate) return true;
  return photoList(photos).length >= gate.min;
}

// Fill from the frequency sheet. Missing ids stay daily.
// Value: "daily" | "weekly" | "monthly"
// or { frequency: "weekly", weekday: 1 }  // 1 = Monday … 7 = Sunday
// or { frequency: "monthly", monthDay: 1 }
export const TASK_FREQUENCY = {};

export function frequencySpec(taskOrId) {
  const id = typeof taskOrId === "string" ? taskOrId : taskOrId?.id;
  const fromTask = typeof taskOrId === "object" && taskOrId?.frequency ? taskOrId.frequency : null;
  const raw = TASK_FREQUENCY[id] ?? fromTask ?? "daily";
  if (typeof raw === "string") return { frequency: raw };
  return {
...
export function frequencyOf(taskOrId) {
  return frequencySpec(taskOrId).frequency;
}

export function periodKey(frequency, date = new Date()) {
...
export function isCompletionLive(taskOrId, iso, now = new Date()) {
  if (!iso) return false;
  const t = new Date(iso);
  if (Number.isNaN(t.getTime())) return false;
  const freq = frequencyOf(taskOrId);
  return periodKey(freq, t) === periodKey(freq, now);
}

export function liveDoneMap(raw, now = new Date()) {
  const out = {};
  for (const [id, iso] of Object.entries(raw || {})) {
    if (isCompletionLive(id, iso, now)) out[id] = iso;
  }
  return out;
}

export function pruneReviewChecks(reviewChecks, now = new Date()) {
  const out = {};
  for (const [cat, locs] of Object.entries(reviewChecks || {})) {
    out[cat] = {};
    for (const [loc, done] of Object.entries(locs || {})) {
      out[cat][loc] = liveDoneMap(done, now);
    }
  }
  return out;
}

export function pruneCompletions(completions, now = new Date()) {
  const out = {};
  for (const [empId, done] of Object.entries(completions || {})) {
    out[empId] = liveDoneMap(done, now);
  }
  return out;
}

export function taskIsVisible(task, doneIso, now = new Date()) {
  if (!task) return false;
  const spec = frequencySpec(task);
  const freq = spec.frequency;
  if (freq === "daily") return true;
  const complete = isCompletionLive(task, doneIso, now);
  if (freq === "weekly" && spec.weekday != null) {
    if (isoWeekday(now) === spec.weekday) return true;
    return !complete;
  }
  if (freq === "monthly" && spec.monthDay != null) {
    if (now.getUTCDate() === spec.monthDay) return true;
    return !complete;
  }
  return true;
}

export function visibleTasks(tasks, doneMap, now = new Date()) {
  const done = liveDoneMap(doneMap, now);
  return (tasks || []).filter((t) => taskIsVisible(t, done[t.id], now));
}

export const CATEGORY_REVIEWERS = {};

export const REVIEW_CATEGORIES = Object.keys(CATEGORY_REVIEWERS);

function userMatchesOwner(user, owner) {
  if (!user || !owner) return false;
  const username = (user.username || "").toLowerCase();
  const name = (user.name || "").toLowerCase();
  return username === owner.username.toLowerCase() || name === owner.name.toLowerCase();
}

export function isDedicatedReviewer(user) {
  return Object.values(CATEGORY_REVIEWERS).some((owner) => userMatchesOwner(user, owner));
}

export function isCategoryReviewer(user, category) {
  const owner = CATEGORY_REVIEWERS[category];
  if (!owner) return true;
  return userMatchesOwner(user, owner);
}

export function taskIdsForUser(user) {
  if (!user || user.role !== "employee") return [];
  const dedicated = isDedicatedReviewer(user);
  return TASKS.filter((t) => {
    const owner = CATEGORY_REVIEWERS[t.category];
    if (dedicated) return !!owner && userMatchesOwner(user, owner);
    return !owner;
  }).map((t) => t.id);
}
...
export function reviewDoneMap(reviewChecks, category, location) {
  return liveDoneMap(reviewChecks?.[category]?.[location] || {});
}

export function reviewProgress(reviewChecks, category, location) {
  const done = reviewDoneMap(reviewChecks, category, location);
  const tasks = visibleTasks(tasksForCategory(category), done);
  const completed = tasks.filter((t) => done[t.id]).length;
  return { total: tasks.length, completed, done };
}

export const ISSUE_CATEGORIES = ["Washroom", "Pantry", "Common Areas", "Infra & Safety", "Soft Services", "Private Cabins", "Lifts", "Meeting Rooms", "Cafe"];
...
export const ISSUE_DELETE_WINDOW_MS = 2 * 60 * 60 * 1000;

export function canDeleteIssue(issue, now = Date.now()) {
  if (!issue?.createdAt) return false;
  const created = new Date(issue.createdAt).getTime();
  if (Number.isNaN(created)) return false;
  return now - created < ISSUE_DELETE_WINDOW_MS;
}

export const VISITOR_DELETE_WINDOW_MS = 3 * 60 * 60 * 1000;

export function canDeleteVisitor(visitor, now = Date.now()) {
  if (!visitor?.createdAt) return false;
  const created = new Date(visitor.createdAt).getTime();
  if (Number.isNaN(created)) return false;
  return now - created < VISITOR_DELETE_WINDOW_MS;
}
...
const DEMO_ISSUES = [
  ...
  {
    employeeId: "e10",
    location: "ECE House, Connaught Place",
    category: "Common Areas",
...
export function getCentreTeam(user, employees = [], targetLocation = null) {
  if (!user) return [];
  const isCM = user.designation === "cm";
  const loc = targetLocation || (user.location !== "All centres" ? user.location : null);
  let team = [user];

  if (isCM) {
    if (loc) {
      const centreSupervisors = employees.filter(
        (e) => e.location === loc && (e.supervisorId === user.id || e.id === user.id)
      );
      team = [user, ...centreSupervisors];
    } else {
      const mySupervisors = employees.filter(
        (e) => e.supervisorId === user.id || (user.location && user.location !== "All centres" && e.location === user.location)
      );
      team = [user, ...mySupervisors];
    }
  } else if (user.supervisorId) {
    const myLoc = loc || user.location;
    const myCM = employees.find(
      (e) => e.id === user.supervisorId || (e.designation === "cm" && (e.location === myLoc || employees.some((s) => s.supervisorId === e.id && s.location === myLoc)))
    );
    const coSupervisors = employees.filter(
      (e) => myLoc && myLoc !== "All centres" && e.location === myLoc && (e.id === user.id || (user.supervisorId && e.supervisorId === user.supervisorId))
    );
    team = [user, ...(myCM ? [myCM] : []), ...coSupervisors];
  } else if (loc && loc !== "All centres") {
    team = employees.filter((e) => e.location === loc);
...
export function computeCentreScore(centreEmployees, completions, customTasks = TASKS, now = new Date()) {
  if (!centreEmployees || centreEmployees.length === 0) {
    return { uniqueDoneCount: 0, totalTasks: 0, scorePct: 0, individualStats: [], uniqueDoneMap: {} };
  }

  const allAssignedTaskIds = new Set();
  centreEmployees.forEach((emp) => {
    taskIdsForUser(emp).forEach((id) => allAssignedTaskIds.add(id));
  });

  const masterTasks = customTasks.filter((t) => allAssignedTaskIds.has(t.id));
  const uniqueDoneMap = {};

  const individualStats = centreEmployees.map((emp) => {
    const done = liveDoneMap(completions?.[emp.id] || {}, now);
    const taskIds = taskIdsForUser(emp);
    const empTasks = customTasks.filter((t) => taskIds.includes(t.id));
    const vis = visibleTasks(empTasks, done, now);
    const completed = vis.filter((t) => done[t.id]).length;

    Object.entries(done).forEach(([taskId, iso]) => {
      if (!uniqueDoneMap[taskId]) uniqueDoneMap[taskId] = iso;
    });

    return { emp, total: vis.length, completed, pct: vis.length ? Math.round((completed / vis.length) * 100) : 0 };
  });

  const visibleMasterTasks = visibleTasks(masterTasks, uniqueDoneMap, now);
  const uniqueDoneCount = visibleMasterTasks.filter((t) => uniqueDoneMap[t.id]).length;
  const totalTasks = visibleMasterTasks.length;
  const scorePct = totalTasks > 0 ? Math.round((uniqueDoneCount / totalTasks) * 100) : 0;

  return { uniqueDoneCount, totalTasks, scorePct, individualStats, uniqueDoneMap };
}

export function computeCMScore(cm, employees, completions, customTasks = TASKS, now = new Date()) {
  const supervisors = employees.filter((e) => e.supervisorId === cm.id);

  if (supervisors.length === 0) {
    const singleStats = computeCentreScore([cm], completions, customTasks, now);
    return {
      scorePct: singleStats.scorePct,
      uniqueDoneCount: singleStats.uniqueDoneCount,
      totalTasks: singleStats.totalTasks,
      groups: [{ location: cm.location || "Direct Centre", employees: [cm], stats: singleStats }],
      supervisors: [],
      individualStats: singleStats.individualStats,
      uniqueDoneMap: singleStats.uniqueDoneMap,
    };
  }

  const locationGroups = {};
  supervisors.forEach((sup) => {
    const loc = sup.location || cm.location || "Default Centre";
    if (!locationGroups[loc]) locationGroups[loc] = [];
    locationGroups[loc].push(sup);
  });

  if (cm.location && cm.location !== "All centres" && !locationGroups[cm.location]) {
    locationGroups[cm.location] = [];
  }

  const uniqueDoneMap = {};
  const groups = Object.entries(locationGroups).map(([loc, emps]) => {
    const team = Array.from(new Set([cm, ...emps]));
    const stats = computeCentreScore(team, completions, customTasks, now);
    Object.assign(uniqueDoneMap, stats.uniqueDoneMap);
    return { location: loc, employees: emps.length > 0 ? emps : [cm], stats };
  });

  const totalTasks = groups.reduce((sum, g) => sum + g.stats.totalTasks, 0);
  const uniqueDoneCount = groups.reduce((sum, g) => sum + g.stats.uniqueDoneCount, 0);
  const scorePct = totalTasks > 0 ? Math.round((uniqueDoneCount / totalTasks) * 100) : 0;
  const individualStats = groups.flatMap((g) => g.stats.individualStats);

  return { scorePct, uniqueDoneCount, totalTasks, groups, supervisors, individualStats, uniqueDoneMap };
}
